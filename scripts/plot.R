...
if (length(args)!=1) {
  stop("usage: Rscript workdir")
} else if (length(args)==1) {
  print(paste("cancer score txt dir:",args[1]))
}

workdir=args[1]
cancername <- paste(workdir,"/Cancer_score_disease.txt",sep = "")
normalname <- paste(workdir,"/Cancer_score_control.txt",sep = "")
...
ggsave(filename = paste(workdir,"/03-deepcat-output/boxplot.pdf",sep = ""))
dev.off()
...
pdf(paste(indir,"/03-deepcat-output/auc.pdf",sep = ""))  # 需要提前指定普通绘图路径和文件名
plot(ROC2, 
     col="red",   #曲线的颜色
